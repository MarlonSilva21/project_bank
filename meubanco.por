programa
{
	...
	funcao contaPoupanca()
		{
			inteiro diaAniversarioPoupanca = 30, diaDehoje, movimento = 0
			//caracter c, d
			cadeia desejo, c , d
			
			tela1()
			
			escreva("\nDeseja fazer crédito [C] ou débito [D]:  ")
			leia(desejo)

			se(desejo == "c" ou desejo == "C")
			{
				creditopop()
			}
			senao se
			(desejo == "d" ou desejo == "D")
			{
				debitopop()
			}
			faca {
			...
			senao{
				...
				escreva("\nVoltar para tela inicial [S] ou [N]")
				leia(voltarpop)
				
			}
			
			enquanto (voltarpop != 's')
			contaPoupanca()	
	...
	funcao tela4(){
	 ...
	 funcao debitopop(){
		...
		leia(valorMovimentado)
		saldopo = saldopo - valorMovimentado
 ...
 * 
 * @POSICAO-CURSOR = 10586; 
 * @DOBRAMENTO-CODIGO = [273];
 * @PONTOS-DE-PARADA = ;
